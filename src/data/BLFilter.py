...
class Filter:
    ...
    def filter(self, data, filt_func):
        ...
        window_type = 'fixed' if (N - self.target_record_size >= self.window_size - 1) else 'variable'
        # logger.info('Applying a {} {} dp {} filter to data with {} dp.'.format(window_type, self.window_size,
        #                                                                        filt_func.__name__, len(data)))
        # apply the filter!
        filt_ = list(filt_func(data, window_size=self.window_size, window_type=window_type))
