...
# Use a short mission for testing: dorado 2010.341.00 http://stoqs.mbari.org/p/SVGDeLc
# The lopc_dorado_processing fixture will process the mission, all other TEST_ variables
# are the same as for Dorado389 2011.256.02
LOPC_MISSION = "2010.341.00"  # http://stoqs.mbari.org/p/SVGDeLc
LOPC_START_YEAR = 2010
...
def lopc_dorado_processing():
    ...
    ns.base_path = os.getenv("BASE_PATH", BASE_PATH)
    ns.auv_name = TEST_VEHICLE
    ns.mission = LOPC_MISSION
    ns.start_year = LOPC_START_YEAR
    # There are several options that need to be set to run the full processing
    ns.clobber = False
    proc.commandline = "args set in conftest.py::lopc_dorado_processing()"
    ns.local = True
    ns.noinput = True
    ns.noreprocess = False
    ns.use_portal = False
