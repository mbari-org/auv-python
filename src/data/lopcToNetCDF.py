...
def readCframeData(binFile):
    ...
    try:
        ...
        if hasMBARICframeData:
            ...
            if debugLevel >= 2:
                logger.debug("Parse from C Frame esecs = %f" % esecs)
            return esecs
        else:
            logger.info(
...
def unpackLOPCbin(binFile, opts, textFile=None):
    ...
    dataStructure["binSizeList"] = numpy.array(
    ...
    while True:
        ...
        if frameID == "L":
            ...
            sepCountArray = numpy.zeros(
                len(dataStructure["binSizeList"]), dtype="int32"
            )
            i = 0
            for c in countList:
                sepCountArray[i] = c
            ...
            if debugLevel >= 1:
                logger.debug(
                    ...
                    % (dataStructure["lFrameCount"], esecs)
                )
            cFrameEsecs = esecs
        ...
        if dataStructure["lFrameCount"] > lastLframeCount + binningInterval:
            ...
            LCcount = mepData.countLC(
                opts.LargeCopepod_AIcrit,
                opts.LargeCopepod_ESDmin,
                opts.LargeCopepod_ESDmax,
            )
...
def constructTimestampList(
    binFile, recCount=None, sampleCountList=None, cFrameEsecsList=None
):
    ...
    if sampleCountList != None:
        ...
        subSampledCorrectedSampleCountList = correctedSampleCountList[
            dataStructure["lFrameCountWrittenList"]
        ]
    ...
    writtenSampleCounts = list(
        numpy.array(dataStructure["sampleCountWrittenList"], dtype="int32") - 1
            ...
            if i + 1 < len(spike_indx):
                ...
                for ix in range(indx, spike_indx[i + 1]):
                    ...
                    sampleCountList[ix] = interpolate(
                        sampleCountList, ix, lookingRange, notSpikeValues, spikeValue
                    )
...
def closeNetCDFFile(ncFile, tsList, cFrameEsecsList):
    ...
    ncFile.variables["correctedSampleCountList"][:] = dataStructure[
        "correctedSampleCountList"
    ]
...
"""
    ...
    if opts.bin_fileName and opts.netCDF_fileName:
        ...
        (tsList, cFrameEsecsList) = constructTimestampList(
            binFile,
            sampleCountList=dataStructure["sampleCountList"],
            cFrameEsecsList=dataStructure["cFrameEsecsList"],
        )
