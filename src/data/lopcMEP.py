...
class MepData:
	...
	def build(self, previousMEP = None):
		...
		for n, p, e, l, s in zip(self.n, self.p, self.e, self.l, self.s):
			logger.debug("build(): n = %s, p = %s, e = %s, l = %s, s = %s" % (n, p, e, l, s))
			if n == 1:
	...
	def count(self, binSizeList, countList = None):
		...
		for mep in self.mepList:
			...
			logger.debug("count(): increment index = %d for mep.esd = %f" % (indx, mep.esd))
			countList[indx] += 1
	...
	def countLC(self, aiCrit = 0.6, esdLowCrit = 1100.0, esdHiCrit = 1700.0):
		...
		lcCount = 0
		logger.debug("countLC(): aiCrit = %f,  esdLowCrit %f, esdHiCrit = %f" % (aiCrit, esdLowCrit, esdHiCrit))
		for mep in self.mepList:
	...
	def __repr__(self):
		...
		str += "\nNumber of MEPs = %d\n" % len(self.aiArray)
		str += "ai:   mean = %f, min = %f, max = %f, std = %f\n" % (self.aiArray.mean(), self.aiArray.min(), self.aiArray.max(), self.aiArray.std())
		str += "esd:  mean = %f, min = %f, max = %f, std = %f\n" % (self.esdArray.mean(), self.esdArray.min(), self.esdArray.max(), self.esdArray.std())
...
if __name__ == '__main__':
	...
	logging.getLogger("MEP").setLevel(logging.DEBUG)	# Set debug for additional output from the methods
	##logging.getLogger("MEP").setLevel(logging.INFO)
